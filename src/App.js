...
function App() {
  ...
  function getBounds(data) {
    ...
    data.forEach(function (d) {
      ...
      bounds.minC = Math.min(bounds.minC || Infinity, d.concentration); // Add minC
      ...
      >
        ...
        <div style={{ width: "100%", height: "calc(100% - 9em)" }}>
          ...
          <div style={{ width: "100%", height: "calc(100% - 2em)" }}>
            <ColorLegend colorRange={colorRange} bounds={bounds} />
          </div>
        </div>
      </div>
      <div>
        <label htmlFor="thresholdSlider">
          Concentration Threshold: {threshold.toFixed(2)}
        </label>
        <input
          id="thresholdSlider"
          type="range"
