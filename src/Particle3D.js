...
export default function Particle3D(props) {
  ...
  useEffect(() => {
    if ((props.bounds !== undefined) & (camera !== undefined)) {
      ...
      const thresholdValue = props.threshold * bounds.maxC; // Calculate threshold based on user input
      const filteredData = props.data.filter(
        (d) => d.concentration >= thresholdValue
      );

      console.log(`Filtered Particles: ${filteredData.length}`);
  ...
  }, [camera, props.data, props.threshold]);
  ...
  useEffect(() => {
    ...
    const saturatedColorScale = d3
      .scaleSymlog()
      .domain([0, bounds.maxC]) // thresholdValue instead of 0
